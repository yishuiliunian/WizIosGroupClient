...
{
    return [NSString stringWithFormat:@"%@%@%@",objguid,kbguid,accountUserId];
};
...
{
    WizLogInfo(@"download error %@,%@",obj.strGuid,obj.strTitle);
}
...
{
    return [self getSyncStatueForKey:WizSyncStatueGroupKey(kbguid,accountUserId)] != WizApiStatueNormal;
}
- (void) refreshGroup:(NSString *)kbguid accountUserId:(NSString *)accountUserId
{
    if (![self isRefreshingGroup:kbguid accountUserId:accountUserId]) {
