...
- (BOOL) updateWizSearch:(NSString *)keywords notesNumber:(NSInteger)notesNumber isSerchLocal:(BOOL)isSearchLocal kbguid:(NSString *)kbguid accountUserId:(NSString *)accountUserId
{
    __block BOOL isSuccess;
    if ([self searchDataFromDb:keywords]) {
        [self.queue inDatabase:^(FMDatabase *db) {
            isSuccess = [db executeUpdate:@"update Wiz_Search set SEARCH_NOTE_COUNT=?, SEARCH_EDIT_DATE=?, SEARCH_ISLOCAL=? ,SEARCH_KBGUID=?, SEARCH_ACCOUNTUSERID=? where SEARCH_KEYWORDS = ?",[NSNumber numberWithInteger:notesNumber], [[NSDate date] stringSql], [NSNumber numberWithBool:isSearchLocal], keywords,kbguid,accountUserId];
        }];
    }
    else
    {
        [self.queue inDatabase:^(FMDatabase *db) {
            isSuccess = [db executeUpdate:@"insert into Wiz_Search (SEARCH_NOTE_COUNT, SEARCH_EDIT_DATE, SEARCH_ISLOCAL,SEARCH_KEYWORDS,SEARCH_ACCOUNTUSERID, SEARCH_KBGUID) values(?,?,?,?,?,?)",[NSNumber numberWithInteger:notesNumber], [[NSDate date] stringSql], [NSNumber numberWithBool:isSearchLocal], keywords, accountUserId, kbguid];
        }];
    }
    return isSuccess;
}
...
- (NSArray*) allWizSearchsByKbguid:(NSString *)kbguid accountUserId:(NSString *)accountUserId
{
    __block NSMutableArray* allSearchs = [NSMutableArray array];
    [self.queue inDatabase:^(FMDatabase *db) {
        FMResultSet* searchDatas = [db executeQuery:@"select SEARCH_NOTE_COUNT, SEARCH_EDIT_DATE,SEARCH_KEYWORDS, SEARCH_ISLOCAL from Wiz_Search where SEARCH_ACCOUNTUSERID =? and SEARCH_KBGUID=?",accountUserId,kbguid];
