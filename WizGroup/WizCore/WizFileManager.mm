...
static const char* CWizLocalSaveFileName = "temp.ziw";
static NSString* const OWizLocalSaveFileName = @"temp.ziw";
...
- (NSString*) downloadObjectTempFilePath:(NSString*)objGuid accountUserId:(NSString *)userId
{
    ...
    return [objectPath stringByAppendingPathComponent:OWizLocalSaveFileName];
}
...
{
    NSString* documentSourcePath = [self wizLocalObjectFilePath:documentGuid accountUserId:accountUserId];
...
-(NSString*) createZipByPath:(NSString *)objectPath
{
    ...
    NSString* zipPath = [objectPath stringByAppendingPathComponent:OWizLocalSaveFileName];
    ZipArchive* zip = [[ZipArchive alloc] init];
    BOOL ret;
...
{
    NSString* accountPath = [self accountPathFor:accountUserId];
    if ([kbGuid isEqualToString:WizStdStringToNSString(WizGlobalPersonalKbguid)]) {
