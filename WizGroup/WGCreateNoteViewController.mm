...
- (void) saveTheDocument
{
    ...
    NSString* oKbGuid = WizStdStringToNSString(self.kbGuid);
    NSString* oAccountUserId = WizStdStringToNSString(self.accountUserId);
    WizModule::WIZDOCUMENTDATA doc;
    doc.strGUID = [documentGuid UTF8String];
    doc.strTitle  = WizNSStringToStdString(title);
    doc.nLocalChanged = WizEditDocumentTypeAllChanged;
    doc.strDateCreated = WizGetCurrentTimeSQLString();
    doc.strDateModified = WizGetCurrentTimeSQLString();
    doc.nServerChanged = NO;
    ...
    [[WizFileManager shareManager] updateEditingIndexFile:bodyText kbguid:oKbGuid accountUserId:oAccountUserId];
    [[WizFileManager shareManager] updateEditingMobildeFile:bodyText kbguid:oKbGuid accountUserId:oAccountUserId];
    [[WizFileManager shareManager] endEditingDocument:documentGuid kbguid:oKbGuid accountUserId:oAccountUserId];
    NSString* filePath = [[WizFileManager shareManager] downloadObjectTempFilePath:documentGuid accountUserId:WizStdStringToNSString(self.accountUserId)];
    NSString* md5 = [WizGlobals fileMD5:filePath];
    doc.strDataMd5 = WizNSStringToStdString(md5);
    WizMetaDb db(dbPath.c_str());
    if (db.updateDocument(doc)) {
        NSLog(@"add succeed!");
    }
}
