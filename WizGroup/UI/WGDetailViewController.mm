...
- (void) addTagTreeNodeToParent:(const WizModule::WIZTAGDATA&)tag   rootNode:(TreeNode*)root  allTags:(const WizModule::CWizTagDataArray&)allTags
{
    TreeNode* node = [[TreeNode alloc] init];
    node.title = WizStdStringToNSString(tag.strName);
    node.keyString = WizStdStringToNSString(tag.strGUID);
    node.isExpanded = NO;
    node.strType = WizTreeViewTagKeyString;
    if (tag.strParentGUID.empty()) {
        [root addChildTreeNode:node];
    } 
    else
    {
        TreeNode* parentNode = [root childNodeFromKeyString:node.keyString];
        if(nil != parentNode)
        {
        ...
        {
            WizModule::WIZTAGDATA tagData;
            for (WizModule::CWizTagDataArray::const_iterator itor = allTags.begin(); itor != allTags.end(); itor++) {
                if (itor->strGUID == tag.strParentGUID) {
                    tagData = *itor;
                    break;
                }
            }
            if (!tagData.strGUID.empty()) {
                [self addTagTreeNodeToParent:tagData rootNode:root allTags:allTags];
                parentNode = [root childNodeFromKeyString:WizStdStringToNSString(tagData.strGUID)];
                [parentNode addChildTreeNode:node];
            }
...
- (void) reloadTagRootNode
{
    ...
    if (metaDb.allTagsForTree(tagsArray)) {
        [rootTreeNode removeAllChildrenNodes];
        for (WizModule::CWizTagDataArray::const_iterator itor = tagsArray.begin(); itor != tagsArray.end(); itor++) {
            [self addTagTreeNodeToParent:*itor rootNode:rootTreeNode allTags:tagsArray];
        }
    }
}
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    NSInteger listType = WGListTypeRecent;
    if (indexPath.section == WGFolderListIndexOfUserTree) {
        TreeNode* node = [self.needDisplayNodesArray objectAtIndex:indexPath.row];
